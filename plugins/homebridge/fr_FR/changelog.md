...
* Mise en place d'une historisation des config homebridge afin de pouvoir revenir à une ancienne config fonctionnant (si on a tout planté dans l'interface config-ui-x par exemple, ou bien si on a fait une erreur de json dans les plateformes/accessoires supplémentaires). !! PAS ENCORE D'INTERFACE POUR LA RESTAURER, EN COURS...
